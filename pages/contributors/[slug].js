...
function BlogPostPage(props) {
  return (
    <div>
      <h1 className="prose text-gray-600 dark:text-gray-400 mb-16">{props.blog.name}</h1>
      <section
...
export async function getStaticProps(context) {
  ...
  const path = `${process.cwd()}/contributors/${slug}.md`;
...
export async function getStaticPaths(context) {
  ...
  const path = `${process.cwd()}/contributors`;
